...
export default function CustomerApplicationsPage() {
  ...
  React.useEffect(() => {
    if (typeof window !== 'undefined') {
      ...
      if (params.get('tab') === 'pending' || params.get('status') === 'pending') {
  ...
  const displayedApplications =
    activeTab === 'all' ? myApplications : pendingApplications;
  ...
  return (
    <AppShell allowedRoles={['customer']}>
      ...
      {/* Tabs: All Application vs Pending (From User Diagram) */}
      <div className="flex items-center justify-between gap-4 border-b border-slate-200 dark:border-slate-800 pb-2">
        <div className="flex items-center gap-2 p-1 rounded-2xl bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-700">
          ...
          >
            <span>All Application / બધી અરજીઓ</span>
            <span className="px-2 py-0.5 rounded-full text-[10px] bg-slate-200 dark:bg-slate-700">
          ...
          >
            <span>Pending / પેન્ડિંગ</span>
            <span className="px-2 py-0.5 rounded-full text-[10px] bg-amber-100 dark:bg-amber-900/40 text-amber-700 dark:text-amber-300">
          ...
          <Card>
            <CardContent className="p-8 text-center space-y-3">
              ...
              <h3 className="text-sm font-bold text-slate-900 dark:text-white">
                {activeTab === 'pending'
                  ? (language === 'gu' ? 'કોઈ પેન્ડિંગ અરજીઓ નથી' : 'No Pending Applications')
              ...
              <p className="text-xs text-slate-500 max-w-md mx-auto">
                {language === 'gu'
                  ...
                  : 'You do not have any applications in this status. Visit the HY-TECH center desk to apply for government schemes.'}
              </p>
            </CardContent>
          </Card>
        ) : (
