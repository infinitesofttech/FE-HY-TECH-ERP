...
export function WhatsAppButton({ number, size = 'sm', className = '', tooltip = true }: WhatsAppButtonProps) {
  ...
  const handleClick = (e: React.MouseEvent<HTMLAnchorElement>) => {
    e.preventDefault();
    e.stopPropagation();
    // Named window 'whatsapp_web' — browser tracks this tab.
    // First click: opens 1 new tab named 'whatsapp_web'.
    // Every subsequent click: NAVIGATES inside the same tab (no duplicate tabs).
    // NOTE: 'noopener' must NOT be used — it breaks named-window tracking.
